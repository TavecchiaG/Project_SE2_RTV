...
First, the “Ride Controller” will perform the proper algorithm developed to find the right PowerGrid in order to reach the purpose of this functionality. Second,it will perform a query on the database in order to retreive the information about the location of the chosen Power Grid; Once that is done, the controller will interface with the APIs provided by Google Maps to compute the road.
